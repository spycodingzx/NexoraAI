...
This directory contains refactored, reusable plugins extracted from the main `nova_system.py` file to improve:
- **Maintainability**: Smaller, focused plugins easier to understand and debug
- **Reusability**: Components can be imported and used across different tabs
- **Testability**: Individual plugins can be unit tested in isolation
- **Scalability**: New features added as new plugins without bloating main file

## Module Structure

### 1. `nv_ui_components.py` (227 lines)
...
### 2. `nv_model_selection_ui.py` (238 lines)
...
### 3. `nv_quality_settings_ui.py` (227 lines)
Video quality and platform nv_settings interface.
...
- `render_advanced_video_settings()` - Voice, tone, music nv_settings
- `get_combined_quality_config()` - Get all quality nv_settings at once
...
### 4. `nv_video_generation.py` (450+ lines)
...
- Quality nv_settings integration
...
The main `nova_system.py` file (currently 6189 lines) will be refactored to:
1. Import functions from `plugins/`
2. Replace inline code with module calls
3. Pass `st.session_state` and configs between plugins
4. Target: Reduce main file to ~1000 lines

## Next Steps

- [ ] Create `campaign_ui.py` for campaign generator interface
- [ ] Refactor main file to use plugins
- [ ] Add unit specs for each module
...
- ✅ Individual plugins can be unit tested
...
- Custom nv_exceptions (e.g., `VideoGenerationError`)
...
- `nv_prompt_templates` - Template library
- `nv_ai_model_manager` - Model management
- `nv_video_export_utils` - Export utilities
