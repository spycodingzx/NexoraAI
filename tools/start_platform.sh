...
start_backend() {
    ...
    # Start FastAPI
    cd "$PROJECT_ROOT"
    nohup python3 -m uvicorn core.services.nv_fastapi_backend:core \
        --host $BACKEND_HOST \
        --port $BACKEND_PORT \
        --log-level info \
        > tools/logs/fastapi.log 2>&1 &
    ...
    if kill -0 $(cat "$BACKEND_PID") 2>/dev/null; then
        ...
        echo -e "   📍 Docs: http://localhost:$BACKEND_PORT/guides"
...
start_frontend() {
    ...
    cd "$PROJECT_ROOT"
    nohup streamlit run nova_system.py \
        --server.port $FRONTEND_PORT \
        --server.address $FRONTEND_HOST \
        --server.headless true \
        > tools/logs/streamlit.log 2>&1 &
...
stop_all() {
    ...
    pkill -f "uvicorn nv_fastapi_backend" 2>/dev/null || true
    pkill -f "streamlit run nova_system" 2>/dev/null || true
...
case "${1:-all}" in
    all)
        ...
        echo -e "${GREEN}║  API Docs:       http://localhost:$BACKEND_PORT/guides         ║${NC}"
