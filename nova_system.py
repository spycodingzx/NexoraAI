...
# Add core directories to path for Streamlit Cloud (symlinks may not work)
...
from core.tabs.nv_abp_config import AppConfig
...
from core.tabs.nv_abp_ui_components import render_about_guide, render_command_line_guide
...
# ========================================
# Only import tab plugins when they're actually needed
# This reduces initial core load time by 40-60%
...
# APP CONFIGURATION
# ========================================
# AppConfig is imported from nv_abp_config.py
...
try:
    from nv_performance_optimizations import (
        LazyLoader,
        PerformanceMonitor,
        fragment_render,
        get_printify_api,
    ...
    def render_performance_settings():
        st.info(
            "⚙️ Performance nv_settings module not available. Install 'nv_performance_optimizations' for advanced options."
        )


# Import platform integrations (API tracking, session persistence, performance boost)
try:
    from core.services.nv_platform_integrations import (
    ...
    def render_full_usage_dashboard():
        st.info(
            "📊 Usage dashboard requires 'nv_platform_integrations' module for API tracking and analytics."
        )
...
from core.services.nv_playground_models import (
    EDITING_MODELS,
    IMAGE_MODELS,
    MARKETING_MODELS,
    MODEL_3D,
...
try:
    from nv_enhanced_features import (
...
try:
    from nv_smart_dashboard_widget import ActivityFeed, NotificationCenter, SmartDashboard
...
try:
    from core.services.nv_unified_session_manager import UnifiedSessionManager as SessionStateManager
    from core.services.nv_unified_session_manager import render_session_manager_modal
...
# ========================================
# Caching functions are imported from nv_abp_utils.py
from core.tabs.nv_abp_utils import (
...
from core.tabs.nv_abp_state import init_session_defaults
...
try:
    from core.tabs.nv_abp_onboarding import check_and_show_onboarding, render_getting_started_sidebar
...
if PLATFORM_INTEGRATIONS_AVAILABLE:
    nv_platform_integrations = init_all_integrations()
    logging.info("✅ Platform integrations initialized")
else:
    nv_platform_integrations = {}

from core.services.nv_unified_session_manager import UnifiedSessionManager as SessionManager
...
from core.utils.nv_cross_page_state import (
    TaskStatus,
    get_state_manager,
...
# Lazy import for nv_campaign_generator_service (takes 0.6s)
...
def _get_campaign_generator():
    ...
    if _campaign_generator_module is None:
        from nv_campaign_generator_service import EnhancedCampaignGenerator
...
# Lazy import for nv_chat_assistant (takes 1.6s to import due to nv_otto_engine chain)
# Import only when actually used
_chat_assistant_module = None


def get_chat_assistant():
    """Lazy load nv_chat_assistant module on first use."""
    global _chat_assistant_module
    if _chat_assistant_module is None:
        from core.services.nv_chat_assistant import render_autonomous_todo, render_chat_interface
...
try:
    from core.services.nv_task_queue_engine import EnhancedTaskQueue, render_enhanced_task_queue
...
try:
    from core.services.nv_shortcuts_manager import (
        BUTTON_SIZES,
        BUTTON_STYLES,
        SHORTCUT_CATEGORIES,
        SHORTCUT_ICONS,
...
from core.services.nv_unified_agent_builder import render_unified_agent_builder
from core.services.nv_unified_session_manager import (
    initialize_session_persistence,
    render_session_manager_ui,
    track_generated_file,
)

# Lazy load nv_video_export_utils (takes 5.5s due to moviepy import)
...
def _get_video_export_utils():
    ...
    if _video_export_module is None:
        from nv_video_export_utils import (
...
from nv_ai_model_manager import ModelFallbackManager, ModelPriority, QualityAssessor, VideoModel
from nv_prompt_templates import PromptEnhancer, PromptTemplateLibrary

from assets.nv_brand_template_generator import generate_dynamic_cta_text

# Lazy load nv_platform_helpers (takes 0.5s due to nv_printify import)
...
# Wrapper functions for nv_platform_helpers
...
# The plugins package imports moviepy which is slow (~2.8s)
_modules_cache = None


def _get_modules():
    """Lazy load plugins package on first use."""
    global _modules_cache
    if _modules_cache is None:
        from plugins import (  # Video Generation; AI Generation (Replicate API); Audio Processing; File Utilities
            MUSIC_PROMPTS,
            VOICE_MAP,
            add_cta_card,
            cleanup_directory,
...
# Wrapper functions for lazy-loaded plugins
...
# ========================================
from core.tabs.nv_abp_sidebar import render_sidebar
...
with st.spinner("Loading tab..."):
    ...
    if renderer:
        try:
            ...
            st.info(
                "💡 Tip: Check 'Manage core' logs for details, or try selecting a different tab."
            )
            import traceback
