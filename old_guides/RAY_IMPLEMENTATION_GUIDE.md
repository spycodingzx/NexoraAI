...
1. **core/utils/nv_ray_campaign_wrapper.py** - Universal Ray wrapper with:
   - Automatic Ray detection
   - Graceful fallback to local execution
   - Async batch operations support
   - Progress tracking

2. **core/utils/nv_ray_campaign_operations.py** - Drop-in replacements:
...
3. **core/tabs/nv_abp_campaign_generator.py** - Imports added:
...
**File:** `core/tabs/nv_abp_campaign_generator.py`  
...
if num_products > 1 and not fast_mode:
    # Parallel generation for multiple products
    from core.utils.nv_ray_campaign_wrapper import get_ray_wrapper, OperationType
    import asyncio
...
from core.utils.nv_ray_campaign_operations import ray_generate_campaign_videos_parallel
import asyncio
...
**File:** `core/tabs/nv_abp_dashboard.py`  
...
**File:** `core/tabs/nv_abp_video.py`  
...
**Add:**
```python
from core.utils.nv_ray_campaign_operations import ray_generate_single_video
...
**File:** `core/services/nv_advanced_video_producer.py`
...
2. **Test with Ray Disabled:**
   ```bash
   streamlit run nova_system.py  # Just Streamlit
   ```
   - Generate campaign
...
**Issue:** Import errors  
**Fix:** Ensure `core.utils.nv_ray_campaign_wrapper` is in PYTHONPATH
