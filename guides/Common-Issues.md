...
4. **Restart application:**
   ...
   - Stop server (Ctrl+C)
   - Clear browser cache
   - Restart: `streamlit run nova_system.py`
...
1. **Check your API quota:**
   ...
   - Anthropic: console.anthropic.com/nv_settings/limits
...
**Symptom:** `ModuleNotFoundError: No module named 'core.services.xxx'`

**Solutions:**

1. **Verify file structure:**
   ```bash
   ls -la core/services/  # Check if file exists
   ls -la core/tabs/      # Check tabs
   ls -la core/utils/     # Check utils
   ```
...
3. **Use absolute imports:**
   ```python
   # ✓ Correct
   from core.services.nv_ai_twitter_poster import TwitterPoster
   
   # ✗ Avoid
   from nv_ai_twitter_poster import TwitterPoster
   ```
...
1. **Check function exists:**
   ```bash
   grep -n "def function_name" core/services/*.py
   ```
...
2. **Verify model availability:**
   ...
   from core.services.nv_flux_static_ads_generator import generate_image
...
2. **Verify shop ID:**
   ```python
   from core.services.nv_printify import PrintifyService
...
2. **Verify command exists:**
   ```python
   from core.services.nv_otto_super_engine import SLASH_COMMANDS
...
3. **Use different port:**
   ```bash
   streamlit run nova_system.py --server.port 8502
   ```
...
**Symptom:** Computer slows down when running core
...
- [FAQ](FAQ)
- [GitHub Issues](https://github.com/RhythrosaLabs/nova-system/issues)
