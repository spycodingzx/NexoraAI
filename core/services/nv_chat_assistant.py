...
from core.services.nv_chat_assistant_history import (
    ChatHistoryManager,
    get_chat_history_manager,
    render_chat_history_sidebar,
)
from core.services.nv_chat_assistant_tasks import AutonomousTaskQueue, TaskQueueItem
...
# Lazy imports for heavy plugins
_replicate_api = None
...
def _get_chat_action_executor():
    ...
    if _chat_action_executor is None:
        from nv_chat_actions import ChatActionExecutor
...
def _get_credential_manager():
    ...
    if _credential_manager is None:
        from nv_credential_manager import get_credential_manager
...
def _get_otto_engine():
    ...
    if not _otto_loaded:
        try:
            from core.services.nv_otto_engine import (
                OttoEngine,
                OttoKnowledgeBase,
                TaskPlan,
                get_knowledge_base,
...
from plugins.nv_orchestrator import MultiAgentOrchestrator


class ChatAssistant:
    """
    AI-powered chat assistant with knowledge of all core features,
    ...
    def __init__(
        ...
        self.nv_credential_manager = get_credential_manager()
...
"""
    ...
    def build_knowledge_base(self) -> str:
        ...
        knowledge.append(
            "  - nova_system.py: Main Streamlit core with 8 horizontal tabs"
        )
        ...
        knowledge.append("\nCore Services:")
        knowledge.append("  - nv_api_service.py: ReplicateAPI unified client")
        ...
        knowledge.append("  - nv_advanced_video_producer.py: 4-phase video production")
        ...
        knowledge.append("  - nv_campaign_generator_service.py: 12-step campaign workflow")
        knowledge.append("    * EnhancedCampaignGenerator with magic-marketer patterns")
        knowledge.append("  - nv_chat_assistant.py: This AI assistant with knowledge base")
        knowledge.append("  - session_manager.py: Auto-save/load session persistence")

        knowledge.append("\nIntegration Services:")
        knowledge.append("  - nv_printify.py: PrintifyAPI - products, blueprints, upload")
        knowledge.append("  - nv_shopify_service.py: ShopifyAPI - blog publishing")
        knowledge.append("  - nv_youtube_upload_service.py: YouTubeUploadService - OAuth + upload")
        knowledge.append("  - nv_social_media_ad_service.py: SocialMediaAdService - ads generation")
        knowledge.append("  - background_removal_service.py: BackgroundRemovalService")

        knowledge.append("\nHelper Modules:")
        knowledge.append("  - nv_unified_agent_builder.py: Node-based workflow builder")
        knowledge.append("  - nv_image_manager.py: Image processing utilities")
        knowledge.append("  - nv_blog_generator.py: Blog post generation")

        knowledge.append("\nKey Functions & Where to Find Them:")
        knowledge.append(
            "  - Campaign generation: nova_system.py Tab 0 (lines ~850-1900)"
        )
        knowledge.append(
            "  - Video production: nova_system.py Tab 5 (lines ~2850-3200)"
        )
        knowledge.append("  - Product mockups: Printify integration (lines ~1250-1400)")
        knowledge.append("  - ControlNet usage: nv_api_service.py (lines ~407-590)")
        knowledge.append(
            "  - Background removal: nova_system.py (lines ~1100-1250)"
        )
        knowledge.append(
        ...
        try:
            ...
            for service, keywords in service_keywords.items():
                if any(keyword in task_lower for keyword in keywords):
                    if self.nv_credential_manager.has_credentials(service):
            ...
            enhanced_task = task
            if service_detected:
                cred_context = self.nv_credential_manager.get_login_context(service_detected)
...
IMPORTANT: Use the credentials provided above to log in automatically. The user should NOT have to provide login information."""
                ...
                available_services = self.nv_credential_manager.list_available_services()
...
If this task requires logging in to any service, check if credentials are available and use them automatically."""
    ...
    async def process_message(self, user_message: str, chat_history: List[Dict]) -> str:
        ...
        try:
            ...
            # ==========================================
            if is_action_request and not is_question and self.nv_otto_engine:
                ...
                plan = self.nv_otto_engine.create_plan(user_message)
                ...
                # Execute the plan
                completed_plan = await self.nv_otto_engine.execute_plan(plan, progress_callback)
            ...
            if is_action_request and not is_question:
                logger.info(f"🎯 Action request (legacy nv_orchestrator): {user_message[:50]}...")
                nv_orchestrator = MultiAgentOrchestrator(self.replicate)
                context = {
                    "task": user_message,
                ...
                results = await nv_orchestrator.orchestrate(user_message, context)
            ...
            credential_context = self.nv_credential_manager.get_full_context()
            ...
            if active_assistant_id and active_assistant_id != "otto_default":
                try:
                    from core.services.nv_custom_assistants import PRESET_ASSISTANTS
...
"""
                    ...
                    # If nv_custom_assistants not available, use default
                    system_prompt = self._get_default_otto_prompt(
                        credential_context, knowledge_base
                    )
            else:
...
- ✍️ Writer - Copy, tools, blog posts
...
"""
    ...
    def list_agents_command(self) -> str:
        ...
        from nv_agent_builder import Agent
...
/* Ensure the chat input nv_container in sidebar stays at bottom */
...
def _initialize_apis():
    ...
    apis = {"nv_printify": None, "shopify": None, "youtube": None}

    try:
        token = os.getenv("PRINTIFY_API_TOKEN") or st.session_state.get("printify_token")
        if token:
            from core.services.nv_api_service import PrintifyAPI

            apis["nv_printify"] = PrintifyAPI(token)
    except Exception:
        pass
    ...
    try:
        ...
        if token and store:
            from core.services.nv_shopify_service import ShopifyService
        ...
        **Upload images or documents to teach Otto context about your business, products, assets, or anything else.**
        ...
        with c1:
            fact = st.text_input(
                ...
                placeholder="e.g., Our assets color is blue...",
...
def render_chat_interface(key_suffix: str = "sidebar"):
    ...
    assistant.action_executor.nv_printify = printify_api
    ...
    # Create a nv_container for messages with fixed height for scrolling
    if is_fullscreen:
