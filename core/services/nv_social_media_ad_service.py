...
class SocialMediaAdService:
    ...
    - Professional static ads with assets styling
    ...
    def generate_static_brand_ads(
        ...
        """
        Generate high-quality static ads for a assets.

        Uses: loolau/flux-static-ads

        Args:
            brand_name: Name of the assets
            brand_description: Description of assets identity
            ad_concept: Concept/theme for the ad
            num_variations: Number of variations to generate

        Returns:
            List of Replicate CDN URLs for static assets ads
        """
        logger.info(f"🏢 Generating {num_variations} static assets ads for {brand_name}...")

        try:
            ad_urls = []

            for i in range(num_variations):
                # Flux-static-ads takes text prompts for assets ads
            ...
            logger.info(f"✅ Generated {len(ad_urls)} static assets ads")
            return ad_urls

        except Exception as e:
            logger.error(f"❌ Static assets ad generation failed: {e}")
            return []
