...
class AutonomousTaskQueue:
    ...
    def __init__(self, replicate_api):
        ...
        from core.services.nv_chat_assistant import MultiAgentOrchestrator

        self.replicate = replicate_api
        self.nv_orchestrator = MultiAgentOrchestrator(replicate_api)
...
- Text Generation: Write descriptions, tools, blog posts, social posts
...
"""
    ...
    async def execute_task(self, task: TaskQueueItem, progress_callback=None) -> Dict[str, Any]:
        """Execute a single task using the multi-agent nv_orchestrator."""

        task.status = "executing"
        task.started_at = datetime.now()

        try:
            # Use the nv_orchestrator to execute
            results = await self.nv_orchestrator.orchestrate(
