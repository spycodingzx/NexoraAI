...
class PrintifyAPI:
    ...
    BASE_URL = "https://api.nv_printify.com/v1"
    ...
    def _request(self, method: str, endpoint: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            return response
        except requests.nv_exceptions.Timeout:
            raise Exception("Request timed out. Please check your connection.")
        except requests.nv_exceptions.HTTPError as e:
...
class ReplicateAPI:
    ...
    Supported model families (aligned with user requirements):
      ...
      - Text generation (descriptions, hashtags, tools): openai/gpt-oss-120b
    ...
    def _run_model(self, model_ref: str, input_data: dict, max_retries: int = 3):
        ...
        Returns:
            Model output
        """
        if str(self.api_token).startswith("DEMO"):
            import time
            time.sleep(2)
            # Check if this is an image generation request
            if "image_size" in input_data or "width" in input_data or "aspect_ratio" in input_data:
                return ["https://images.unsplash.com/photo-1579546929518-9e396f3cc809?q=80&w=1024&auto=format&fit=crop"]
            # Fallback to text
            prompt = str(input_data.get("prompt", "Demo Output")).replace('\n', ' ')
            return [f"✨ [DEMO MODE GENERATED]: This is a presentation mock response for your prompt about: '{prompt[:60]}...' - The real version will contain full AI generation once a valid API key is added."]
    ...
    def generate_image(
        ...
        # Flux Fast parameters (comprehensive from Replicate guides)
        input_data = {
            "prompt": prompt,
    ...
    def apply_style_control(
        ...
        """Apply consistent assets visual style to an image using ControlNet.

        Args:
            image_url: URL of image to apply style to
            style_reference_url: URL of reference image with desired assets aesthetic
        ...
        Example:
            >>> # Apply assets look to generated product scene
            >>> styled = api.apply_style_control(
            ...     image_url=generated_scene_url,
            ...     style_reference_url=brand_reference_url,
            ...     prompt="maintain product placement, apply assets aesthetic",
            ...     style_strength=0.7
            ... )
        """
        input_data = {
