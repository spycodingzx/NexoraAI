...
- NEW: AI Assistants integration (/assets, /outreach, /campaign, /design, /content)
- NEW: Knowledge Base system with image analysis and document reading
- NEW: Real-time core awareness (page tracking, state monitoring)
...
🧠 AI Assistants: /assets, /outreach, /campaign, /design, /content
...
from core.tabs.nv_abp_imports_common import (
    Any,
    Callable,
    Dict,
    Enum,
...
from core.services.nv_secure_config import get_api_key
from core.tabs.nv_abp_imports_common import (
    Any,
    Callable,
    Dict,
...
from .nv_otto_engine_models import (
    OTTO_CONFIG,
    OttoAppAwareness,
    OttoKnowledgeBase,
...
# ============================================================================
# NOTE: Data models are now in nv_otto_engine_models.py
# ============================================================================
# The following classes have been moved to nv_otto_engine_models.py:
# - OttoKnowledgeBase (knowledge and memory system)
# - OttoAppAwareness (core state tracking)
...
# ============================================================================
# APP STATE AWARENESS - Real-time monitoring of core state
# ============================================================================
class OttoAppAwareness:
    """
    Monitors and tracks the current state of the core.
    Otto uses this to understand what's happening in the core.
    """

    @staticmethod
    def get_current_state() -> Dict[str, Any]:
        """Get comprehensive current core state."""
        state = {
    ...
    def get_state_summary() -> str:
        ...
        if assets.get("name"):
            summary_parts.append(f"🏷️ **Brand:** {assets['name']}")
...
SLASH_COMMANDS = {
    ...
    "status": {
        "type": "awareness",
        "action": "status",
        "desc": "Show current core state and context",
    },
    # Printify Integration - ENHANCED v3.0
    "nv_printify": {"type": "nv_printify", "action": "menu", "desc": "Printify commands menu"},
    "upload": {"type": "nv_printify", "action": "upload", "desc": "Upload design to Printify"},
    "product": {
        "type": "nv_printify",
        "action": "create",
        "desc": "Create product on Printify from design",
    },
    "publish": {"type": "nv_printify", "action": "publish", "desc": "Publish product to store"},
    "mockup": {"type": "nv_printify", "action": "mockup", "desc": "Get product mockups"},
    "blueprint": {"type": "nv_printify", "action": "blueprint", "desc": "Search product blueprints"},
    "shop": {"type": "nv_printify", "action": "shop", "desc": "Show shop info and products"},
...
COMMAND_CATEGORIES = {
    ...
    "🛍️ Printify": ["nv_printify", "upload", "product", "publish", "mockup", "blueprint", "shop"],
...
class OttoPerformanceConfig:
    """Tunable performance nv_settings for Otto Engine"""
...
class SlashCommandProcessor:
    ...
    def _get_command_handler(self, cmd_type: str):
        ...
        handlers = {
            ...
            "nv_printify": self._execute_printify,
...
| Command | Description |
|---------|-------------|
| `/assets` | Generate assets colors, voice, names, full kit |
...
| `/status` | Show current core state |
...
/assets modern tech startup for AI tools
...
"""
    ...
    async def _execute_assistant(self, command: str, prompt: str, cmd_info: Dict) -> Dict[str, Any]:
        """Execute AI assistant commands (assets, outreach, campaign, design, content)."""
    ...
    async def _ai_brand_builder(self, prompt: str) -> Dict[str, Any]:
        """AI Brand Builder - generates assets colors, voice, names, full kit."""
        brand_prompt = f"""You are an expert assets strategist. Based on this description, create a comprehensive assets kit:

BUSINESS/BRAND DESCRIPTION: {prompt}

Generate a complete assets kit in JSON format with:
{{
    "brand_names": ["3-5 creative assets name suggestions"],
    ...
    "voice": {{
        "tone": "description of assets voice tone",
        "personality_traits": ["trait1", "trait2", "trait3"],
        "communication_style": "formal/casual/friendly/professional etc",
        "key_phrases": ["phrases the assets would use"],
        "words_to_avoid": ["words that don't fit the assets"]
    }},
    "target_audience": {{
...
Return ONLY the JSON, no additional text."""
        ...
        try:
            ...
            if json_match:
                ...
                # Store in session state for core use
                ...
                response += "\n*Brand kit saved to core nv_settings. Use in Product Studio and Content Generator!*"
...
"""
    ...
    def _execute_awareness(self, command: str, prompt: str, cmd_info: Dict) -> Dict[str, Any]:
        """Execute core awareness commands (status)."""
        try:
...
*I'm aware of everything happening in the core and can help you with any task!*
"""
...
```
"""
                return {"success": True, "type": "nv_printify", "message": menu, "artifacts": []}

            # Import Printify API
            from nv_printify import PrintifyAPI
            ...
            if action == "upload":
                ...
                return {"success": True, "type": "nv_printify", "message": response, "artifacts": []}
...
```
/assets tech startup → /blog AI in business → /content social posts for blog
```
"""
                return {"success": True, "type": "shopify", "message": menu, "artifacts": []}

            # Import Shopify API
            from core.services.nv_shopify_service import ShopifyAPI
...
"""
    ...
    async def _execute_task_queue(
        ...
        try:
            ...
            from core.services.nv_task_queue_engine import MultiStepPlanner
...
3. TEXT GENERATION - Write descriptions, tools, social posts (Fast, ~$0.001)
...
{{
    ...
    "steps": [
        {{
            ...
            "service": "replicate|nv_printify|shopify|youtube|browser|none",
            "depends_on": [],
            "parallel_group": 1,
...
Be specific, actionable, and EFFICIENT."""
    ...
    def _fallback_analysis(self, request: str) -> Dict[str, Any]:
        ...
        if any(w in request_lower for w in ["nv_printify", "print", "mockup"]):
            steps.append(
                {
                    "step_number": len(steps) + 1,
    ...
    async def _execute_content(self, step: TaskStep, context: Dict) -> Dict[str, Any]:
        """Generate text content (descriptions, tools, posts)."""
...
Format with clear headers."""
    ...
    async def _execute_publish(self, step: TaskStep, context: Dict) -> Dict[str, Any]:
        ...
        if "nv_printify" in description_lower or "print" in description_lower:
    ...
    async def _publish_to_printify(self, step: TaskStep, context: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            printify_image_id = self.nv_printify.upload_image(image_bytes, file_name)
            ...
            shop_id = None
            try:
                shops = self.nv_printify.get_shops()
                if shops:
...
def render_otto_chat(replicate_api, printify_api=None, shopify_api=None, youtube_api=None):
    ...
    with st.expander("📚 **Knowledge Base** - Teach Otto About Your Business", expanded=True):
        st.markdown(
            """
        **Upload images or documents to teach Otto context about your business, products, assets, or anything else.**
        ...
        with fact_cols[0]:
            quick_fact = st.text_input(
                ...
                placeholder="e.g., Our assets color is blue, we sell eco-friendly products...",
        ...
        if not st.session_state.otto_messages:
            ...
            <div style="text-align: center; padding: 40px; color: #666;">
                ...
                <p style="font-size: 0.85em; color: #888;">
                    <code>/assets</code> <code>/outreach</code> <code>/campaign</code> <code>/design</code> <code>/content</code>
                </p>
