...
class OttoTaskQueueExtension:
    ...
    def __init__(self, nv_task_queue_engine):
        """
        Initialize with reference to task queue engine.

        Args:
            nv_task_queue_engine: EnhancedTaskQueue instance
        """
        self.task_queue = nv_task_queue_engine
    ...
    def add_task_from_otto(
        ...
        """
        from nv_task_queue_engine import TaskPriority
...
class OttoDocumentExtension:
    ...
    def __init__(self, base_path: str = "."):
        ...
        self.docs_dir = self.base_path / "guides"
...
class OttoPinterestTikTokExtension:
    ...
    def __init__(self, nv_multi_platform_poster=None):
        """
        Initialize Pinterest/TikTok extension.

        Args:
            nv_multi_platform_poster: MultiPlatformPoster instance
        """
        self.poster = nv_multi_platform_poster
    ...
    async def execute_workflow(
        ...
        try:
            if workflow_type == "content_to_social":
                ...
                doc_result = self.guides.create_document(
...
def enhance_otto_with_extensions(
    otto_super, nv_task_queue_engine=None, nv_multi_platform_poster=None
) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        nv_task_queue_engine: Optional EnhancedTaskQueue
        nv_multi_platform_poster: Optional MultiPlatformPoster
    ...
    if nv_task_queue_engine:
        extensions["tasks"] = OttoTaskQueueExtension(nv_task_queue_engine)

    # Document Extension
    extensions["guides"] = OttoDocumentExtension()

    # Social Extension
    if nv_multi_platform_poster:
        extensions["social"] = OttoPinterestTikTokExtension(nv_multi_platform_poster)

    # Workflow Extension (if we have all pieces)
    if all(k in extensions for k in ["tasks", "guides", "social"]):
        extensions["workflow"] = OttoWorkflowExtension(
            extensions["tasks"], extensions["guides"], extensions["social"]
        )
