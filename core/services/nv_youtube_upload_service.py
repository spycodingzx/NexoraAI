...
Integrates patterns from youtube-automate for nova_system.
"""
...
from core.utils.nv_youtube_helper import get_youtube_service, upload_to_youtube
...
class YouTubeUploadService:
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.info("💡 Run: python nv_setup_youtube.py")
            self.is_authenticated = False
            return False
        except Exception as e:
            logger.error(f"❌ YouTube authentication failed: {e}")
            logger.info("💡 Try running: python nv_setup_youtube.py")
            self.is_authenticated = False
            return False
    ...
    def check_credentials(self) -> dict[str, any]:
        ...
        if client_id and client_secret:
            ...
            result["message"] = (
                "YouTube credentials missing. Add YOUTUBE_CLIENT_ID and YOUTUBE_CLIENT_SECRET to .env or run nv_setup_youtube.py"
            )
        ...
        try:
            if self.authenticate():
                ...
                result["message"] = "Authentication required - run: python nv_setup_youtube.py"
    ...
    def generate_viral_metadata(
        ...
        Args:
            product_name: Product/assets name
