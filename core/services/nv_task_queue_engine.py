...
class Task:
    ...
    # Publishing targets
    publish_to: List[str] = field(default_factory=list)  # nv_printify, shopify, youtube, twitter, etc.
...
class EnhancedTaskQueue:
    ...
    def nv_otto_engine(self):
        ...
        if self._otto_engine is None:
            try:
                ...
                from nv_otto_super_main import OttoSuper

                self._otto_engine = OttoSuper(
                    replicate_api=self.replicate,
                    printify_api=self.nv_printify,
                    shopify_api=self.shopify,
                    youtube_api=self.youtube,
                    nv_task_queue_engine=self,  # Pass self for circular integration
                    nv_multi_platform_poster=None,  # Will be lazy-loaded if needed
                )
                logger.info("✅ Task queue using Otto Super (enhanced)")
            except ImportError:
                # Fallback to basic OttoEngine
                try:
                    from core.services.nv_otto_engine import OttoEngine
    ...
    def ray_manager(self):
        ...
        if self._ray_manager is None and self.enable_ray:
            try:
                from nv_ray_task_wrapper import get_ray_manager
...
- Text Generation: Write descriptions, tools, blog posts
...
- writer: Writes copy, descriptions, tools (can use specific text models)
...
{{
    ...
    "publish_to": ["nv_printify", "shopify", "youtube", "twitter"],
    "model_preferences": {{
    ...
    def _fallback_plan(self, description: str) -> Dict:
        ...
        if any(kw in desc_lower for kw in ["nv_printify", "t-shirt", "mug", "hoodie", "product"]):
            agents.append("publisher")
            publish_to.append("nv_printify")
            steps.append(
                {
                    "step": len(steps) + 1,
    ...
    async def _execute_publish(self, step: TaskStep, context: Dict) -> Dict[str, Any]:
        ...
        if action == "publish_printify" or "nv_printify" in step.description.lower():
    ...
    async def _publish_to_printify(self, step: TaskStep, context: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.nv_printify.upload_image(response.content, file_name)
                ...
                "artifacts": [
                    {
                        ...
                        "metadata": {"service": "nv_printify", "image_id": image_id},
                    }
                ],
...
def render_enhanced_task_queue(
    ...
    .badge-nv_printify { background: #00a86b; color: white; }
                    ...
                    if task.artifacts:
                        ...
                        for artifact in task.artifacts:
                            if artifact.type == ArtifactType.IMAGE and artifact.url:
                                ...
                                # Can't nest expander, use a styled nv_container instead
