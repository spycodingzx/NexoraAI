...
All Replicate API calls, audio mixing, and file I/O are centralized in plugins/ directory.
"""
...
from plugins.nv_ai_generation import (
    generate_background_music,
    generate_script_with_llama,
    generate_voiceover_audio,
    parse_script_segments,
)
from plugins.nv_audio_processing import prepare_background_music
from plugins.nv_file_utils import create_temp_directory

# Import modular functions to eliminate code duplication
from plugins.nv_video_generation import add_cta_card
...
class StaticCommercialProducer:
    ...
    def generate_script(
        ...
        REFACTORED: Now uses plugins.nv_ai_generation.generate_script_with_llama()
        """
        logger.info("🧠 Generating commercial script with Llama...")

        # Ensure tools only reference explicitly-provided product features and do not invent claims
        features_text = ""
        if product_features:
    ...
    def generate_voiceover(
        ...
        REFACTORED: Now uses plugins.nv_ai_generation.generate_voiceover_audio()
        """
    ...
    def generate_background_music(
        ...
        REFACTORED: Now uses plugins.nv_ai_generation.generate_background_music()
        """
    ...
    def create_commercial_from_images(
            ...
            try:
                ...
                if not os.path.exists(img_path):
                    ...
                    if not allow_substitute_visuals:
                        ...
                        # If substitution allowed, use a placeholder image from assets assets
                        from nv_brand_brain import BrandBrain
        ...
        # Apply assets overlay (logo, color bar, aspect ratio) if assets template provided
        try:
            if brand_template:
                from plugins.nv_video_generation import apply_brand_overlay
