...
class ComfyUIExecutor:
    ...
    def _execute_via_replicate(self, workflow_json: Dict) -> Dict:
        ...
        try:
            ...
            # Extract prompts and nv_settings from workflow
            prompts = self._extract_prompts(workflow_json)
            nv_settings = self._extract_settings(workflow_json)
            ...
            if any("flux" in m.name.lower() for m in checkpoint_models):
                ...
                model_input = {
                    ...
                    "width": nv_settings.get("width", 1024),
                    "height": nv_settings.get("height", 1024),
                    "num_inference_steps": nv_settings.get("steps", 30),
                }
            else:
                # Default to SD 1.5
                ...
                model_input = {
                    "prompt": prompts.get("positive", ""),
                    "negative_prompt": prompts.get("negative", ""),
                    "width": nv_settings.get("width", 512),
                    "height": nv_settings.get("height", 512),
                    "num_inference_steps": nv_settings.get("steps", 50),
                }
    ...
    def _extract_settings(self, workflow_json: Dict) -> Dict:
        """Extract generation nv_settings from workflow"""
        nv_settings = {"width": 512, "height": 512, "steps": 20, "cfg": 7.0, "sampler": "euler"}
        ...
        if "nodes" not in workflow_json:
            ...
            for node_id, node_data in workflow_json.items():
                ...
                if "KSampler" in class_type:
                    nv_settings["steps"] = inputs.get("steps", nv_settings["steps"])
                    nv_settings["cfg"] = inputs.get("cfg", nv_settings["cfg"])
                    nv_settings["sampler"] = inputs.get("sampler_name", nv_settings["sampler"])

                elif "EmptyLatentImage" in class_type:
                    nv_settings["width"] = inputs.get("width", nv_settings["width"])
                    nv_settings["height"] = inputs.get("height", nv_settings["height"])
