...
class JobType(str, Enum):
    ...
    VIDEO_GENERATION = "nv_video_generation"
...
@asynccontextmanager
async def lifespan(core: FastAPI):
...
@core.post("/jobs", response_model=JobResponse)
async def submit_job(request: JobSubmitRequest, nv_background_tasks: BackgroundTasks):
    ...
    nv_background_tasks.add_task(job_executor.execute_job, job.job_id)
...
@core.post("/jobs/batch")
async def submit_batch_jobs(request: BatchJobRequest, nv_background_tasks: BackgroundTasks):
...
@core.get("/jobs", response_model=List[JobResponse])
...
@core.get("/jobs/stats", response_model=QueueStats)
...
@core.get("/jobs/{job_id}", response_model=JobResponse)
...
async def generate_image(
    ...
    nv_background_tasks: BackgroundTasks = None,
):
    ...
    return await submit_job(request, nv_background_tasks)


@core.post("/generate/text")
...
if __name__ == "__main__":
    uvicorn.run("nv_fastapi_backend:core", host="0.0.0.0", port=8601, reload=True, log_level="info")
