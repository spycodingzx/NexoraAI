...
from .nv_otto_app_controller import OttoAppController, parse_app_control_intent

# Import all Otto components
from .nv_otto_super_engine import (
    ChatMessage,
    ChatSession,
    IntentParser,
    MemoryManager,
...
from .nv_otto_super_engine_part2 import ContextTracker, ExecutionEngine, ExecutionResult

# Import Otto extensions
from .nv_otto_super_extensions import (
...
class OttoSuper:
    ...
    def __init__(
        ...
        shopify_api=None,
        youtube_api=None,
        nv_task_queue_engine=None,
        nv_multi_platform_poster=None,
        enable_ray=None,
    ):
        ...
        if enable_ray is None:
            try:
                from nv_ray_integration_helpers import is_ray_enabled
        ...
        self.task_queue_ext = OttoTaskQueueExtension(nv_task_queue_engine)
        self.document_ext = OttoDocumentExtension()
        self.social_ext = OttoPinterestTikTokExtension(nv_multi_platform_poster)
        ...
        self.app_controller = OttoAppController(
            ...
            task_queue=nv_task_queue_engine,
            nv_multi_platform_poster=nv_multi_platform_poster,
        )
    ...
    def get_system_status(self) -> dict[str, bool]:
        ...
        return {
            ...
            "nv_printify": bool(self.nv_printify),
...
- Printify: {'✅' if self.nv_printify else '❌'}
...
"""
    ...
    async def _handle_app_control(
        ...
        """Handle core-wide control operations (full platform access)."""
        ...
        try:
            ...
            # Automation
                ...
                return {
                    "success": False,
                    "message": f"❌ Unknown core control intent: {intent_type}",
                }
    ...
    async def _handle_extended_intent(
        ...
        try:
            if action == "add_to_queue":
                ...
                guides = self.document_ext.list_documents()
                doc_list = "\n".join([f"- {d['name']} ({d['size']} bytes)" for d in guides[:20]])
                return {
                    "success": True,
                    "response": f"📄 Available documents ({len(guides)} total):\n\n{doc_list}",
                }
...
def render_otto_super_ui(replicate_api, printify_api=None, shopify_api=None, youtube_api=None):
    ...
    if "otto_super" not in st.session_state:
        with st.spinner("🚀 Initializing Otto Super..."):
            ...
            try:
                from core.services.nv_task_queue_engine import TaskQueueEngine
            ...
            st.session_state.otto_super = OttoSuper(
                ...
                shopify_api=shopify_api,
                youtube_api=youtube_api,
                nv_task_queue_engine=task_queue,
                nv_multi_platform_poster=multi_poster,
            )
