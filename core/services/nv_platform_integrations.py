...
# Import our new plugins
try:
    from core.utils.nv_api_usage_tracker import APIUsageTracker, get_tracker, render_usage_dashboard
...
try:
    from nv_session_persistence import (
        SessionPersistence,
...
try:
    from nv_performance_boost import (
        BackgroundLoader,
        LazyImporter,
        PerformanceMonitor,
        cached_data,
...
def init_all_integrations() -> dict[str, Any]:
    """
    Initialize all integration plugins

    Call this after initialize_session_state() in the main core

    Returns:
        Dict containing initialized components
    """
    result = {"api_tracker": None, "nv_session_persistence": None, "performance": None}
    ...
    if SESSION_PERSISTENCE_AVAILABLE:
        try:
            if "nv_session_persistence" not in st.session_state:
                persistence = SessionPersistence()
                st.session_state.nv_session_persistence = persistence

                # Check for crash recovery on startup
                if persistence.has_recovery_data():
                    st.session_state.show_recovery_dialog = True

            result["nv_session_persistence"] = st.session_state.nv_session_persistence
        except Exception as e:
...
def render_integrations_sidebar():
    ...
    Add this to the sidebar section of your main core
    """
    st.markdown("---")
    ...
    if SESSION_PERSISTENCE_AVAILABLE:
        with st.expander("💾 Session", expanded=False):
            persistence = st.session_state.get("nv_session_persistence")
            if persistence:
...
def render_recovery_check():
    ...
    Call this early in the core rendering
    """
    if SESSION_PERSISTENCE_AVAILABLE:
...
INTEGRATION_EXAMPLE = """
# Add these imports near the top of nova_system.py:
