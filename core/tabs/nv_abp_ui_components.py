...
def render_about_guide():
    ...
    with st.expander("📚 Page-by-Page Feature Breakdown"):
        ...
        - **Brand Template Selection**: Choose from 7 assets presets
        ...
        - **Parameter Experimentation**: Fine-tune nv_settings
    ...
    with st.expander("🚀 Advanced Features & Power User Tips"):
        ...
        - **Style Reference**: Apply assets-consistent aesthetics
    ...
    with st.expander("⚙️ Complete Setup Guide - Start to Finish"):
        ...
        **Printify** (for product publishing):
        1. Go to [nv_printify.com](https://nv_printify.com)
        ...
        **Shopify** (for blog publishing):
        ...
        3. Create new core with Admin API access
    ...
    with st.expander("🔧 How the Platform Works (Backend)"):
        ...
        **Core Components**:
        | File | Purpose |
        |------|---------|
        | `nova_system.py` | Main Streamlit core (11K+ lines) |
        | `nv_otto_engine.py` | Otto AI core with caching & parallel execution |
        | `nv_otto_super_engine.py` | Hyperintelligent intent parsing |
        | `nv_api_service.py` | Unified Replicate/Printify API wrapper |
        | `nv_campaign_generator_service.py` | 12-step campaign orchestration |
        | `nv_advanced_video_producer.py` | 4-phase video production |
        | `nv_multi_platform_poster.py` | Browser automation via Anthropic Claude |
        | `nv_contact_finder_service.py` | AI-powered contact discovery |
        ...
        **Text Generation**:
        ...
        - **Lazy Loading**: Heavy plugins load on-demand
...
| `/ini` | INI | `/ini application nv_settings` |
