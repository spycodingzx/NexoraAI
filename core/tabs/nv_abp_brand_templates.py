...
def render_brand_templates_tab():
    ...
    st.markdown("### Create and manage assets guidelines for consistent styling")

    # AI Brand Builder Section
    with st.expander("🤖 AI Brand Builder", expanded=True):
        st.markdown("**Let AI help you create a complete assets identity**")
                    ...
                    if replicate_token:
                        with st.spinner("🎨 Creating your perfect color palette..."):
                            try:
                                ...
                                color_prompt = f"""You are a assets color expert. Create a 5-color palette for:
            ...
            if st.button("✍️ Generate Brand Voice", use_container_width=True):
                if brand_business:
                    try:
                        ...
                        replicate_token = None
                    if replicate_token:
                        with st.spinner("✍️ Crafting your assets voice..."):
                            try:
                                import replicate

                                client = replicate.Client(api_token=replicate_token)
                                voice_prompt = f"""Create a assets voice guide for:
            ...
            if st.button("🏷️ Generate Brand Name Ideas", use_container_width=True):
                if brand_business:
                    try:
                        ...
                        replicate_token = None
                    if replicate_token:
                        with st.spinner("🏷️ Brainstorming assets names..."):
                            try:
                                import replicate

                                client = replicate.Client(api_token=replicate_token)
                                name_prompt = f"""Generate 10 unique assets name ideas for:
            ...
            if st.button("📋 Generate Full Brand Kit", use_container_width=True, type="primary"):
                if brand_business:
                    try:
                        ...
                        replicate_token = None
                    if replicate_token:
                        with st.spinner("📋 Creating comprehensive assets kit..."):
                            try:
                                import replicate

                                client = replicate.Client(api_token=replicate_token)
                                full_prompt = f"""Create a complete assets identity kit for:
...
Provide a structured assets kit with:
...
Be specific with hex codes and font suggestions."""
        ...
        if st.session_state.get("ai_full_brand_kit"):
            ...
            if st.button("💾 Save as Brand Template", use_container_width=True):
                ...
                new_template = {
                    ...
                    "description": f"AI-generated assets for {brand_business}",
                    "ai_generated": True,
    ...
    # Import assets templates module
    try:
        from core.utils.nv_brand_templates import (
            PRESET_TEMPLATES,
            get_active_template,
        ...
        if active_template:
            st.success(
                f"🎨 Active Brand: **{active_template.name}** - All generations will follow this assets's style"
            )
            ...
            st.info(
                "💡 Select a assets template below to apply consistent styling across all generations"
            )
        ...
        # Fallback: Simple assets selection
