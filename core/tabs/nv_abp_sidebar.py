from core.tabs.nv_abp_imports_common import Path, datetime, os, setup_logger, st, time

# Maintain backward compatibility alias
dt = datetime
logger = setup_logger(__name__)

from core.services.nv_enhanced_features import GlobalSearchManager
from core.services.nv_platform_helpers import _get_replicate_token
from core.services.nv_platform_integrations import render_recovery_check
from core.services.nv_tab_visibility_manager import (
    get_filtered_tabs,
    initialize_tab_visibility,
    render_tab_preferences,
)
from core.utils.nv_performance_optimizations import render_performance_settings

# Import nv_validation utilities
try:
    from core.utils.nv_validation import (
        APIValidator,
        InputValidator,
...
try:
    from core.services.nv_background_tasks import TaskState, get_task_manager
    ...
    TaskState = None
try:
    from core.services.nv_shortcuts_manager import ShortcutsManager
except ImportError:
    ShortcutsManager = None
try:
    from core.utils.nv_api_usage_tracker import api_tracker as _api_tracker_instance
...
def render_sidebar(
    ...
    with st.sidebar:
        ...
        if BACKGROUND_TASKS_AVAILABLE:
            try:
                from core.services.nv_background_tasks import render_task_status_widget
        ...
        with sidebar_tabs[3]:  # Settings
            ...
            with tab1:
                ...
                current_printify = st.session_state.api_keys.get("nv_printify", "")
                ...
                st.markdown("---")
                st.caption("💡 To update credentials, edit your `.env` file and restart the core.")
        ...
        with tab2:
            ...
            with st.expander("🔧 Setup Instructions", expanded=not creds_status["authenticated"]):
                ...
                2. Place it in: `/Users/sheils/repos/nv_printify/`
                        ...
                        if st.button("🐦 Post to Twitter", type="primary"):
                            if test_caption:
                                with st.spinner("Posting to Twitter..."):
                                    try:
                                        from core.services.nv_ai_twitter_poster import AITwitterPoster
                                        ...
                                        st.error(
                                            f"Twitter posting requires nv_ai_twitter_poster module: {e}"
                                        )
                                    except Exception as e:
                                        st.error(f"Error: {e}")
                            else:
                ...
                ANTHROPIC_API_KEY=your_anthropic_key  # Required for AI browser control
                        ...
                        if st.button("💾 Save Twitter Credentials"):
                            if new_twitter_user and new_twitter_pass:
                                ...
                                st.success("✅ Saved! Restart the core to apply.")
                            else:
        ...
        with tab_shortcuts:  # Keyboard Shortcuts
            ...
            # Initialize shortcuts nv_settings
            if "keyboard_shortcuts" not in st.session_state:
                st.session_state.keyboard_shortcuts = {}

            # Check if ShortcutsManager is available (imported at module level)
            try:
                from core.services.nv_shortcuts_manager import ShortcutsManager as SM
        ...
        with tab5:  # Export
            ...
            with export_tabs[0]:
                ...
                with col_fmt1:
                    ...
                    st.caption("Best for importing back into the core")
                    import json
            ...
            with export_tabs[1]:
                ...
                try:
                    if API_USAGE_TRACKER_AVAILABLE:
                        from core.utils.nv_api_usage_tracker import api_tracker
        ...
        with tab6:  # Performance
            ...
            st.markdown("---")
            try:
                from nv_abp_ray_cluster import render_ray_cluster_ui
        ...
        with tab7:  # Voice Control
            ...
            try:
                from nv_simple_voice_control import render_simple_voice_control
                                ...
                                if st.button(
                                    ...
                                    with st.spinner(f"Running {shortcut_name}..."):
                                        try:
                                            ...
                                            from core.services.nv_otto_engine import get_slash_processor
                                            ...
                                            if not replicate_token:
                                                ...
                                                for step in shortcut.get("steps", []):
                                                    ...
                                                    if step["type"] == "generate":
                                                        ...
                                                        if "twitter" in platform and image_path:
                                                            try:
                                                                from core.services.nv_ai_twitter_poster import (
                                                                    AITwitterPoster,
                                                                )
                                                ...
                                                if shortcut.get("nv_settings", {}).get("notify", True):
                                                    st.success("🎉 Done!")
    ...
    with header_cols[4]:
        ...
        try:
            from nv_background_task_manager import render_compact_progress_indicator
