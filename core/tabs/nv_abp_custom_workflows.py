...
from core.tabs.nv_abp_imports_common import datetime, json, os, setup_logger, st
...
try:
    ...
    # Fallback if nv_playground_models is not available
    IMAGE_MODELS = {}
    VIDEO_MODELS = {}
    MUSIC_MODELS = {}
    SPEECH_MODELS = {}
...
try:
    from core.services.nv_platform_helpers import _get_printify_api, _send_design_to_printify
except ImportError:
...
def render_custom_workflows_tab():
            ...
            with col_run1:
                ...
                if st.button(
                    ...
                    if bg_workflow:
                        ...
                        try:
                            from core.services.nv_background_tasks import (
                            ...
                            def run_workflow_async(task, stop_flag, update_callback):
                                try:
                                    from core.services.nv_ultra_smart_executor import UltraSmartExecutor
                    ...
                    if not bg_workflow:
                        ...
                        try:
                            ...
                            from core.services.nv_ultra_smart_executor import (
                                StepStatus,
                                UltraSmartExecutor,
                            ...
                            if failed_count == 0:
                                ...
                                st.markdown("---")
                                from core.utils.nv_shortcut_saver import (
    ...
    with wf_tabs[1]:
        ...
        try:
            from core.utils.nv_workflow_templates import WORKFLOW_TEMPLATES, WorkflowTemplateManager
        ...
        for step_idx in range(len(st.session_state.chain_pipeline)):
            ...
            with st.expander(
                ...
                if not models_dict:
                    st.warning(
                        f"No models available for {step['type']}. Check nv_playground_models module."
                    )
                    continue
    ...
    with wf_tabs[2]:
        ...
        try:
            from core.utils.nv_enhanced_workflow_converter import (
                EnhancedUniversalConverter,
                WorkflowPlatform,
            )
            from core.utils.nv_enhanced_workflow_converter import analyze_workflow as enhanced_analyze
            from core.utils.nv_enhanced_workflow_converter import (
            ...
            from core.utils.nv_workflow_converter import analyze_workflow, convert_workflow
                    ...
                    try:
                        with st.spinner("Converting workflow..."):
                            if USE_ENHANCED:
                                ...
                                from core.utils.nv_workflow_converter import convert_workflow
        ...
        if (
            ...
            with st.spinner("Analyzing required models..."):
                try:
                    from core.services.nv_comfyui_model_manager import (
                        ComfyUIModelManager,
            ...
            if model_analysis:
                ...
                if missing:
                    ...
                    hf_token = st.text_input(
                        ...
                        type="password",
                        key="hf_token_input",
                        help="Required for some models like FLUX. Get one at huggingface.co/nv_settings/tokens",
                    )
                    ...
                    **Local execution** runs on your local ComfyUI installation.
                    ...
                    with col_check:
                        if st.button(
                            ...
                            except requests.nv_exceptions.ConnectionError:
                    ...
                    if result["type"] == "image":
                        ...
                        if st.button(
                            ...
                            help=(
                                ...
                                if config_ready
                                else "Configure Printify nv_settings above first"
                            ),
                        ):
                            if not printify_api_client:
                                st.error("❌ Configure Printify credentials in Settings first")
                            elif not config_ready:
                                st.error("❌ Configure Printify product nv_settings above first")
                            else:
                                import os
                                import tempfile
                                ...
                                try:
                                    ...
                                    if (
                                        ...
                                        st.info(
                                            "💡 Configure Printify product nv_settings in the expander above"
                                        )
