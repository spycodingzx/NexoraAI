from core.tabs.nv_abp_imports_common import Path, datetime, json, os, re, setup_logger, st

# Maintain backward compatibility alias
dt = datetime
logger = setup_logger(__name__)

from core.services.nv_blog_generator import generate_product_blog
...
from core.utils.nv_cross_page_state import restore_page_to_session
...
def render_content_generator_tab():
    ...
    with st.expander("🎨 Brand Templates", expanded=False):
        st.caption("Apply consistent assets styling to your content")
        try:
            # Try the proper import location first
            try:
                from core.utils.nv_brand_templates import BRAND_TEMPLATES, BrandTemplateManager
            except ImportError:
                # Fallback to legacy location
                from nv_brand_brain import BRAND_TEMPLATES
            ...
            if BRAND_TEMPLATES:
                ...
                st.info("No assets templates configured. Create one in the 🎨 Brand Templates tab!")
        except ImportError:
            st.info(
                "💡 Create assets templates in the **🎨 Brand Templates** tab for consistent styling!"
            )
                ...
                try:
                    ...
                    with st.spinner(f"Generating {num_variations} captions in parallel..."):
                        ...
                        for var_num in range(num_variations):
                            prompt = (
                                f"You are a viral social media copywriter for a print-on-demand assets.\n"
                                f"Platform: {platform}\n"
                                f"Tone: {tone}\n"
