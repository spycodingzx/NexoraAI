...
from core.utils.nv_cross_page_state import get_state_manager, restore_page_to_session
from core.utils.nv_playground_ui_helpers import render_file_upload, render_model_parameters
...
def render_playground_tab():
    ...
    if playground_mode == "🖼️ Image Generation":
        ...
        st.markdown("---")
        try:
            from core.services.nv_audio_editor import render_audio_editor_ui

            render_audio_editor_ui()
        except ImportError as e:
            st.error(f"Audio editor not available: {e}")
            st.info("Make sure nv_audio_editor.py is in the project directory")

    elif playground_mode == "📄 Document Editor":
        st.markdown("---")
        try:
            from core.services.nv_document_editor import render_document_editor_ui

            render_document_editor_ui()
        except ImportError as e:
            st.error(f"Document editor not available: {e}")
            st.info("Make sure nv_document_editor.py is in the project directory")

    elif playground_mode == "📊 Spreadsheet":
        st.markdown("---")
        try:
            from core.utils.nv_spreadsheet_viewer import render_spreadsheet_viewer_ui

            render_spreadsheet_viewer_ui()
        except ImportError as e:
            st.error(f"Spreadsheet viewer not available: {e}")
            st.info("Make sure nv_spreadsheet_viewer.py is in the project directory")
