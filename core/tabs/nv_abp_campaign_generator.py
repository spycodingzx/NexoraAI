from core.services.nv_platform_integrations import tracked_replicate_run
...
try:
    from core.services.nv_ai_twitter_poster import post_to_twitter_ai
...
# Import helpers
from core.services.nv_campaign_generator_service import EnhancedCampaignGenerator

# VideoMaker is not a class - nv_videomaker.py is a standalone streamlit core
from core.utils.nv_ray_campaign_operations import show_ray_performance_info
from core.utils.nv_unified_storage import auto_save_generated_content
...
from core.services.nv_platform_helpers import (
...
from plugins import save_text_file
from plugins.nv_file_utils import save_binary_file
...
try:
    ...
    def tracked_replicate_run(client, model, input_params, operation_name=None):
        """Fallback to regular replicate.run if nv_platform_integrations unavailable"""
        return client.run(model, input=input_params)


from core.tabs.nv_abp_campaign_results import render_campaign_complete_summary
...
def run_campaign_generation(
    ...
    auto_publish_printify = auto_publish_settings.get("nv_printify", False)
    ...
    try:
        from core.services.nv_api_service import ReplicateAPI
    ...
    # Use a mutable nv_container for current_step so nested function can modify it
    ...
    if campaign_enabled:
        ...
        with st.spinner("🚀 Initializing campaign generator..."):
            ...
            # Create progress nv_container for detailed updates
            ...
            if extra_ai_steps.get("influencer_outreach"):
                ...
                with st.spinner(f"👥 Finding Real Contacts ({current_extra}/{enabled_count})..."):
                    try:
                        ...
                        from nv_contact_finder_service import ContactFinderService
...
7. Quote templates for founder/assets (genuine, not corporate)
...
Include specific metrics and measurement methodologies."""
        ...
        if product_enabled:
            ...
            def generate_single_product(product_idx: int) -> dict:
                ...
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        # Use fast mode nv_settings if enabled
            ...
            if num_products > 1 and not fast_mode:
                ...
                from core.utils.nv_ray_campaign_wrapper import OperationType, get_ray_wrapper
                                                ...
                                                if all_mockups:
                                                    ...
                                                    if (
                                                        ...
                                                        try:
                                                            with st.spinner(
                                                                ...
                                                                from nv_product_promotional_content import (
                                                                                ...
                                                                                if (
                                                                                    ...
                                                                                    if st.button(
                                                                                        ...
                                                                                        if AI_TWITTER_AVAILABLE:
                                                                                            ...
                                                                                            from nv_social_media_automation import (
                                                    ...
                                                    if lifestyle_mockups:
                                                        ...
                                                        if st.button(
                                                            ...
                                                            from nv_advanced_video_producer import (
                                                            ...
                                                            with st.spinner(
                                                                ...
                                                                try:
                                                                    ...
                                                                    from nv_static_commercial_producer import (
                                                                    ...
                                                                    # Enforce product-fidelity by passing explicit product features and assets template
                                                                    product_features = (
    ...
    if blog_enabled:
        ...
        try:
            from nv_blog_generator import generate_product_blog

            with st.spinner("Generating hero images and HTML export..."):
                # Get product mockup LOCAL PATH for blog hero image (nv_blog_generator needs file path)
    ...
    if video_enabled:
        ...
        try:
            ...
            if results.get("products"):
                first_product = results["products"][0]
                # Get product type from nv_printify config
                printify_config = (
...
CRITICAL: Line 2 must be VERY SHORT (5-8 words only). Keep it SHORT. Natural. Like you're showing a friend something cool. No exclamation points. No hype words. Just chill. Focus on the PRODUCT not just the design."""
            ...
            for i, segment in enumerate(script_segments):
                ...
                if auto_use_hailuo:
                    ...
                    if not segment_image:
                        ...
                        continue
                    from plugins.nv_video_generation import generate_ken_burns_video
            ...
            if auto_publish_youtube:
                ...
                try:
                    from core.services.nv_youtube_upload_service import YouTubeUploadService
                    ...
                    if yt_service.authenticate():
                        ...
                        st.info("💡 Run `python nv_setup_youtube.py` to authenticate")
                except Exception as e:
    ...
    if social_enabled:
        ...
        with st.spinner("Creating social media assets..."):
            try:
                from nv_campaign_helpers import generate_content
                ...
                if products_with_images:
                    ...
                    try:
                        ...
                        if not mockup_source:
                            ...
                            # Check for active assets template
                            brand_name = None
                            active_brand = st.session_state.get("active_brand_template")
                            if active_brand:
                                # Handle both dict and string (assets name only)
                            ...
                            # Determine ad style from assets or default
                            ...
                            # Extract assets colors if assets template is active
                            brand_colors = None
                            active_brand_id = st.session_state.get("active_brand_id")
                            nv_brand_templates = st.session_state.get("nv_brand_templates", [])
                            if active_brand_id and nv_brand_templates:
                                active_brand = next(
                                    (b for b in nv_brand_templates if b.get("id") == active_brand_id),
                                    None,
                                )
                                if active_brand and active_brand.get("colors"):
                                    ...
                                    st.info(
                                        f"🎨 Using assets colors: {brand_colors.get('primary', 'default')}"
                                    )

                            # Use PIL-based text overlay (reliable and professional)
                            try:
                                from nv_flux_static_ads_generator import (
            ...
            try:
                ...
                if not images_to_post:
                    ...
                    if not AI_TWITTER_AVAILABLE:
                        ...
                        from nv_social_media_automation import SocialMediaAutomation
    ...
    if other_platforms_enabled:
        ...
        with st.spinner("Posting to multiple social platforms..."):
            try:
                from nv_multi_platform_poster import MultiPlatformPoster
    ...
    if auto_send_email and email_recipients:
        ...
        with st.spinner("Generating and sending marketing emails..."):
            try:
                from core.services.nv_email_marketing_service import EmailMarketingService
    ...
    if digital_config.get("enabled") and digital_products_enabled:
        ...
        with st.spinner("Generating complete digital products (this may take several minutes)..."):
            try:
                # Get assets template for styling
                ...
                if brand_template:
                    ...
                    st.info(f"📎 Applying assets template: {brand_template.get('name', '')}")
                ...
                if configured_price <= 0:
                    try:
                        from nv_flux_static_ads_generator import calculate_smart_price
                ...
                if product_type in ["ebook", "coloring_book", "course", "comic"]:
                    try:
                        from nv_digital_product_generator import (
                ...
                if product_type == "ebook" and FULL_GENERATOR_AVAILABLE:
                    ...
                    st.info(f"🎓 Generating {digital_config.get('plugins', 4)}-module Course...")
                    generator = CourseGenerator()
                    digital_result = generator.generate_course(
                        topic=concept_input + brand_context,
                        title=title,
                        num_modules=digital_config.get("plugins", 4),
                    ...
                    from core.services.nv_digital_products_service import (
                ...
                if digital_result:
                    ...
                    if auto_publish and digital_result.get("pdf_path"):
                        ...
                        try:
                            from core.services.nv_digital_products_service import DigitalProductsService

                            service = DigitalProductsService()

                            # Build description with assets voice
                            ...
                            if product_type == "ebook":
                                ...
                                desc += f"<p>A comprehensive {digital_config.get('plugins', 4)}-module course.</p>"
                            elif product_type == "comic":
                                desc += (
                                    f"<p>A {digital_config.get('pages', 8)}-page comic book.</p>"
                                )

                            # Add assets hashtags as tags
