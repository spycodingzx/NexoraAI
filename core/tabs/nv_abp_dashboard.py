from core.tabs.nv_abp_imports_common import Path, datetime, json, os, setup_logger, st

# Maintain backward compatibility alias
dt = datetime
logger = setup_logger(__name__)

from core.services.nv_platform_helpers import _get_replicate_token, _render_printify_product_config
from core.utils.nv_cross_page_state import render_campaign_status_banner
...
def render_dashboard_tab(smart_dashboard_available, cross_page_mgr):
    ...
    brand_templates_file = Path(__file__).parent.parent.parent / "nv_brand_templates.json"
    nv_brand_templates = []
    ...
    if brand_templates_file.exists():
        try:
            with open(brand_templates_file) as f:
                ...
                nv_brand_templates = brand_data.get("templates", [])
                active_brand_id = brand_data.get("active_brand")
        except Exception:
            pass

    if nv_brand_templates:
        brand_options = ["None (No assets template)"] + [t["name"] for t in nv_brand_templates]
        brand_values = [None] + [t["id"] for t in nv_brand_templates]
        ...
        with brand_col1:
            selected_brand_idx = st.selectbox(
                ...
                index=default_idx,
                help="Select a assets template to guide all generations - styles, colors, voice, hashtags",
            )
            selected_brand_id = brand_values[selected_brand_idx]
            selected_brand = None
            for t in nv_brand_templates:
        ...
        st.info(
            "💡 Create assets templates in the **Brand Templates** page for consistent styling across all generations!"
        )
        st.session_state["selected_brand_template"] = None
    ...
    with col_pub3:
        ...
        if auto_publish_reddit:
            reddit_subreddits = st.text_input(
                ...
                value="",
                placeholder="e.g., nv_printify, etsy, entrepreneurship",
                help="Be mindful of subreddit rules!",
            )
            st.session_state["reddit_subreddits"] = reddit_subreddits

    # Social media browser nv_settings (shown if any social platform is enabled)
    ...
    with st.expander("⚙️ Advanced Configuration & Model Parameters", expanded=False):
        ...
        # Voice nv_settings (only if voiceover enabled)
        ...
        if model_selection_mode == "🎯 Recommended (Auto)":
            st.info("💡 AI will automatically select the best model based on your nv_settings")
            ...
            from core.services.nv_ai_model_manager import ModelFallbackManager
            ...
            requirements = {
                ...
                "duration": 10,  # Will be updated based on actual nv_settings
            }
            ...
            from core.services.nv_ai_model_manager import ModelPriority
        ...
        if use_prompt_templates:
            from core.utils.nv_prompt_templates import PromptTemplateLibrary
        ...
        with preset_col1:
            platform_preset = st.selectbox(
                ...
                help="Apply platform-optimized nv_settings automatically",
            )
        with preset_col2:
        ...
        with st.container():
            ...
            if use_kling:
                st.caption("Additional Kling nv_settings")
        ...
        with st.container():
            ...
            st.caption("openai/gpt-oss-120b - AI text for tools, descriptions, and content")
    ...
    if start_button:
        from core.tabs.nv_abp_campaign_generator import run_campaign_generation
        ...
        run_campaign_generation(
            ...
            auto_publish_settings={
                "nv_printify": auto_publish_printify,
