...
from core.tabs.nv_abp_imports_common import datetime, os, requests, setup_logger, st

# Maintain backward compatibility alias
dt = datetime
logger = setup_logger(__name__)

from core.services.nv_global_job_queue import JobType, get_global_job_queue
from core.services.nv_tab_job_helpers import are_all_jobs_done, check_jobs_progress, collect_job_results
from core.tabs.nv_abp_utils import cached_scan_files, cached_scan_products
...
def render_file_grid(files, key_prefix, cols_count=4):
    """Render files in a clean grid nv_layout"""
    if not files:
...
def render_file_library_tab():
    ...
    with file_filter_tabs[1]:
        ...
        with kb_tabs[2]:
            ...
            try:
                from core.services.nv_otto_engine import get_knowledge_base
    ...
    with file_filter_tabs[2]:
        ...
        with prod_tabs[1]:
            ...
            try:
                from core.services.nv_api_service import PrintifyAPI as PrintifyClient
                            ...
                            for prod in products[:5]:  # Show first 5
                                with st.expander(f"{prod.get('title', 'Untitled')}"):
                                    ...
                                    with col2:
                                        ...
                                        st.markdown(
                                            f"[View in Printify](https://nv_printify.com/core/store/products/{prod['id']})"
                                        )
                        else:
    ...
    with file_filter_tabs[3]:
        ...
        st.markdown(
            "Browse and manage all your templates: assets guidelines, social media kits, video presets, and workflow templates."
        )
        ...
        with template_tabs[0]:
            ...
            st.markdown("Your saved assets identities with colors, fonts, and style guidelines.")

            # Load assets templates
            templates_file = Path("nv_brand_templates.json")
            nv_brand_templates = []
            if templates_file.exists():
                try:
                    with open(templates_file) as f:
                        data = json.load(f)
                        nv_brand_templates = data.get("templates", [])
                except Exception:
                    pass

            if nv_brand_templates:
                for idx, template in enumerate(nv_brand_templates):
                ...
                st.info("No assets templates yet. Create one in the Brand Templates tab!")
        ...
        with template_tabs[3]:
            ...
            for idx, wf in enumerate(nv_workflow_templates):
    ...
    with file_filter_tabs[4]:
        ...
        try:
            from core.services.nv_chat_assistant import get_chat_history_manager
