...
Provides type-safe configuration with nv_validation and environment-based nv_settings.
Uses Pydantic for nv_validation and supports multiple config sources.
"""
...
class Settings(BaseSettings):
    """Main nv_settings class combining all configuration"""
    ...
    def get_api_key(self, key_name: str) -> Optional[str]:
        ...
        """
        # Try to get from structured nv_settings first
...
def get_settings() -> Settings:
    """
    Get cached nv_settings instance

    This function is cached to ensure we only load nv_settings once,
...
def get_api_key(key_name: str) -> Optional[str]:
    ...
    This provides backward compatibility with the old nv_secure_config pattern.
    """
    nv_settings = get_settings()
    return nv_settings.get_api_key(key_name)
