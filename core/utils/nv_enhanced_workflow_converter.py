...
SEMANTIC_PATTERNS = {
    ...
    r"(generat|creat|produc).*(video|animation|clip)": "nv_video_generation",
    ...
    r"(upload|push|send).*(nv_printify)": "upload_printify",
...
class EnhancedUniversalConverter:
    ...
    def detect_platform(self, workflow_json: Any) -> WorkflowPlatform:
        ...
        # Make.com: has flow.plugins structure
        if isinstance(workflow_json, dict):
            if "flow" in workflow_json and "plugins" in workflow_json.get("flow", {}):
        ...
        # Windmill: has value.plugins structure
        if isinstance(workflow_json, dict):
            if "value" in workflow_json and "plugins" in workflow_json.get("value", {}):
    ...
    def _convert_n8n(self, workflow_json: dict) -> tuple[dict, WorkflowAnalysis]:
        ...
        for node in nodes:
            ...
            # Update analysis
            if mapping["capability"] == "nv_ai_generation":
    ...
    def _convert_make(self, workflow_json: dict) -> tuple[dict, WorkflowAnalysis]:
        ...
        plugins = flow.get("plugins", [])

        analysis = WorkflowAnalysis(
            platform=WorkflowPlatform.MAKE,
            node_count=len(plugins),
        )

        steps = []
        for i, module in enumerate(plugins, 1):
        ...
        analysis.description = f"Make.com workflow with {len(plugins)} plugins"
    ...
    def _convert_windmill(self, workflow_json: dict) -> tuple[dict, WorkflowAnalysis]:
        ...
        analysis.description = f"Windmill flow with {len(plugins)} plugins"
    ...
    def _convert_unknown(self, workflow_json: Any) -> tuple[dict, WorkflowAnalysis]:
        ...
        if isinstance(workflow_json, dict):
            ...
            for key in ["steps", "nodes", "actions", "plugins", "tasks"]:
