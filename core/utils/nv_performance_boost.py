...
class LazyImporter:
    ...
    def register_many(self, plugins: List[Union[str, tuple]]) -> "LazyImporter":
...
def setup_lazy_imports() -> LazyImporter:
    """Setup lazy imports for commonly used heavy plugins"""
    lazy = LazyImporter()
    lazy.register_many(
        [
            ("numpy", "np"),
...
def cached_resource(ttl_seconds: Optional[int] = None, validate: Optional[Callable] = None):
    """
    Enhanced st.cache_resource with nv_validation
...
class FragmentManager:
    ...
    def render(self, key: str, *args, **kwargs):
        ...
        # Create nv_container if not exists
        if frag.nv_container is None:
            frag.nv_container = st.container()

        with frag.nv_container:
            frag.render_func(*args, **kwargs)

        frag.last_rendered = datetime.now()

    def render_with_key(self, key: str, container_key: str = None):
        """Render fragment with explicit nv_container key for rerun isolation"""
        if key not in self._fragments:
            raise KeyError(f"Fragment '{key}' not registered")

        frag = self._fragments[key]
        nv_container = st.container(key=container_key or f"frag_{key}")
...
def optimize_streamlit_config():
    ...
    Call this at the very start of your core
    """
