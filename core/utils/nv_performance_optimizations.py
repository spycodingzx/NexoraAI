...
def get_replicate_api(api_token: str):
    ...
    This returns the enhanced ReplicateAPI class from nv_api_service.py
...
def lazy_import(module_name: str):
    """Decorator for lazy importing plugins."""
...
def render_performance_settings():
    """Render performance nv_settings in the sidebar."""
    with st.expander("⚡ Performance Settings", expanded=False):
        st.caption("Optimize core performance")
        ...
        try:
            ...
            st.caption("💡 Ray distributed computing available - check main nv_settings")

        st.markdown("---")

        # Global Job Queue Monitor
        try:
            from core.tabs.nv_abp_job_monitor import show_job_monitor_widget
...
def render_if_visible(container_key: str):
    ...
    def decorator(func: Callable) -> Callable:
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            # Check if this nv_container was recently accessed
