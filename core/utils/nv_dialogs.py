...
from core.services.nv_data_models import PriceRule, ProductTemplate, ScheduledJob
...
class TemplateDialog(QDialog):
    ...
    def _setup_ui(self):
        nv_layout = QVBoxLayout(self)

        # Name
        nv_layout.addWidget(QLabel("Template Name:"))
        self.name_input = QLineEdit()
        self.name_input.setPlaceholderText("e.g., 'Minimalist Mug Collection'")
        nv_layout.addWidget(self.name_input)

        # Product Type
        nv_layout.addWidget(QLabel("Product Type:"))
        self.type_combo = QComboBox()
        self.type_combo.setEditable(True)
        self.type_combo.addItems(
            ["mug", "t-shirt", "poster", "hoodie", "canvas", "sticker", "phone case", "tote bag"]
        )
        nv_layout.addWidget(self.type_combo)
        ...
        h.addStretch()
        nv_layout.addLayout(h)

        # Prompt Template
        nv_layout.addWidget(QLabel("Prompt Template (use {prompt} as placeholder):"))
        ...
        # Tags
        nv_layout.addWidget(QLabel("Tags (comma-separated):"))
        self.tags_input = QLineEdit()
        self.tags_input.setPlaceholderText("e.g., 'art, modern, gift, unique'")
        nv_layout.addWidget(self.tags_input)

        # Collection
        nv_layout.addWidget(QLabel("Collection Name (optional):"))
        self.collection_input = QLineEdit()
        nv_layout.addWidget(self.collection_input)

        # Options
        self.auto_publish_check = QCheckBox("Auto-publish products")
        nv_layout.addWidget(self.auto_publish_check)

        self.marketing_check = QCheckBox("Generate marketing assets")
        self.marketing_check.setChecked(True)
        nv_layout.addWidget(self.marketing_check)

        # Buttons
        buttons = QDialogButtonBox(QDialogButtonBox.Save | QDialogButtonBox.Cancel)
        buttons.accepted.connect(self.accept)
        buttons.rejected.connect(self.reject)
        nv_layout.addWidget(buttons)
...
class PriceRuleDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        nv_layout.addWidget(QLabel("Set base prices for each product type:"))
        ...
        nv_layout.addWidget(self.table)
...
class ScheduleDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        # Date/Time
        nv_layout.addWidget(QLabel("Schedule Date & Time:"))
        self.datetime_edit = QDateTimeEdit()
        self.datetime_edit.setDateTime(QDateTime.currentDateTime().addSecs(3600))
        self.datetime_edit.setCalendarPopup(True)
        nv_layout.addWidget(self.datetime_edit)
...
class LocalModelsDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        if self.manager.is_installed():
            ...
            nv_layout.addWidget(installed_label)

            close_btn = QPushButton("Close")
            close_btn.clicked.connect(self.accept)
            nv_layout.addWidget(close_btn)
            return
        ...
        nv_layout.addWidget(models_group)
        ...
        nv_layout.addWidget(self.progress_text)
