"""
Input nv_validation and API connection testing utilities
Provides real-time nv_validation, pre-flight checks, and test connection functionality
"""
...
class ValidationResult:
    """Result of a nv_validation check"""
...
class APIValidator:
    """Validates API keys and specs connections"""
    ...
    def test_printify_token(token: str, shop_id: str) -> ValidationResult:
        ...
        try:
            ...
            response = requests.get(
                f"https://api.nv_printify.com/v1/shops/{shop_id}.json", headers=headers, timeout=10
            )
...
def display_validation_result(result: ValidationResult, show_success: bool = True):
    """Display nv_validation result in Streamlit UI"""
    if result.is_valid:
        if show_success:
...
def create_test_connection_button(
    ...
    """Create a test connection button with automatic nv_validation display"""
